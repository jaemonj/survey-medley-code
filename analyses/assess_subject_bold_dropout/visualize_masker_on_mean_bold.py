...
def extract_subid(path):
    after_sub = path.split('sub-')[1]  # everything after 'sub-'
    subid = after_sub.split('/')[0]  # up to first '/'
    return subid
...
output_pdf = '/oak/stanford/groups/russpold/data/uh2/aim1/analysis_code/survey_medley_code/analyses/assess_subject_bold_dropout/mask_and_mean_QA.pdf'

mask_output_dir = '/oak/stanford/groups/russpold/data/uh2/aim1/derivatives/survey_medley_results/assess_subject_bold_dropout/nifti_masker_masks'
...
with PdfPages(output_pdf) as pdf:
    for bold in bold_files_4d:
        ...
        subid = extract_subid(bold)
        mask_img.to_filename(f'{mask_output_dir}/sub-{subid}.nii.gz')
