...
def get_confounds_data(confounds_file):
    ...
    # high VIFs -> only include trans and rot and their derivatives, and cosine regressors
    confounds = confounds_df.filter(regex='^(cosine00|cosine01|cosine02|trans|rot)').copy()
...
def make_desmat_contrasts(root, task, events_file, 
    ...
    """
    from utils_lev1.first_level_designs_including_survey_medley import make_task_desmat_fcn_dict
...
def get_files(root, subid, task):
    ...
    # ))
    files['events_file'], file_missing['event_file_missing'] = check_file(glob.glob(
        f'{root}/BIDS/sub-s{subid}/ses-[0-9]/func/*{task}*modified*.tsv'
    ))
...
def get_parser():
    ...
    parser.add_argument(
        ...
        choices=['stroop', 'ANT', 'CCTHot', 'stopSignal', 'twoByTwo', 'WATT3',
                 'discountFix', 'DPX', 'motorSelectiveStop', 'surveyMedley'],
    ...
    parser.add_argument(
        'regress_rt',
        choices=['no_rt', 'rt_uncentered', 'rt_centered', 'rt_duration'],
        help=('Use to specify how rt is/is not modeled. rt_uncentered and rt_centered model RT with parametric modulation. '
              'If rt_centered is used the mean RT across all subjects is used for centering. '
              'rt_duration models RT with duration modulation (only works with surveyMedley).')
    )
    parser.add_argument(
        '--omit_deriv',
        action='store_true',
...
if __name__ == "__main__":
    ...
    outdir = (f'/oak/stanford/groups/russpold/data/uh2/aim1/derivatives/'
        f'output_surveyMedley_noderivs_rerun/{task}_lev1_output/')
    ...
    if not os.path.exists(contrast_dir):
        ...
        os.makedirs(f'{contrast_dir}/zstat_estimates')
    ...
    if not any_fail and qa_only == False:
        ...
        for con_name, con in contrasts.items():
            ...
            zstat_filename = (f'{contrast_dir}/zstat_estimates/task_{task}_contrast_{con_name}_sub_'
                f'{subid}_rtmodel_{regress_rt}_stat'
                f'_zstat.nii.gz')
            zstat_est = out.compute_contrast(con, output_type = 'z_score')
            zstat_est.to_filename(zstat_filename)
