...
def make_contrast_map(
    sub_id, events_files, questionnaires, question_output_path, activation_maps, cfg
):
    ...
    for i in range(int(design_matrix.shape[1] / 2)):
        ...
        contrast_map_dir = (
            cfg.output_root
            / f'within_subject_brain_behavior_by_questionnaire/within_subject_results/{sub_id}'
        )
        contrast_map_dir.mkdir(parents=True, exist_ok=True)
        nf.save(
            contrast_map,
            f'{contrast_map_dir}/{design_matrix.columns[i]}_behavioral_measures_effect_size_sub_{sub_id}.nii.gz',
        )
